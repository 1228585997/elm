<template>
  <div class="city_container">
    ...
    <ul class="citylist_ul clearfix">
      <li v-for="(item, index) in citylist" :key="index" @click="nextpage(item,index,item.geohash)">
        <h4 class="pois_name ellipsis">
          <router-link
            :to="{name:'shoplist',query:{address:item.address,geohash:item.geohash}}"
    ...
    <ul class="getpois_ul citylist_ul clearfix" v-if="listplaceHistory">
      <li v-for="(item,index) in listplaceHistory" :key="index">
        <h4 class="pois_name ellipsis">
          <router-link
            :to="{name:'shoplist',query:{latitude:item.latitude,longitude:item.longitude,name:item.address}}"
          >{{item.name}}</router-link>
        </h4>
        <p class="pois_address ellipsis">
          <router-link
    ...
    <!-- 清空所有 -->
    <footer v-if="historytitle&&citylist.length" class="clear_all_history" @click="clearAll">清空所有</footer>
...
export default {
  ...
  methods: {
    ...
    onsearch() {
      this.$http
        .get(
          `https://elm.cangdu.org/v1/pois?city_id=${this.id}&keyword=${this.inputVaule}&type=search`
        )
        .then(data => {
