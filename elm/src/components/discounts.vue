<template>
  <div id="discounts">
    <div>
      <router-view></router-view>
    </div>
    <headtop>
      <template v-slot:discounts>
        <router-link to="/profile">
          <van-icon name="arrow-left" class="arrow-lefts" size="20px" />
        </router-link>
        <div class="title_head ellipsis">
          <span class="title_text">我的优惠</span>
        </div>
      </template>
    </headtop>
    <div class="category_title">
      <div v-for="(i,$index) in arr_tab" @click="index=$index" class="tab" :key="$index">
        <span :class="$index==index?'ou':''">{{i}}</span>
      </div>
    </div>
    <div class="hongbao_container">
      <!-- 红包 -->
      <div v-if="index==0">
        <div class="hongbao_bigc">
          <div class="hongbao_title">
            <div class="total_number">
              有
              <span>3</span> 个红包即将到期
            </div>
            <div class="hongbao_description">
              <img
                src="data:image/png;base64,"
              />
              <a class="hongbao_detail">
              <router-link to="/discounts/hbDescription">红包说明</router-link>
              </a>
            </div>
          </div>
          <div class="hongbao_list_ul">
            <div v-for="(i,$index) in arr" :key="$index" class="hongbao_list_li">
              <div class="list_item">
                <div class="list_item_left">
                  <p>
                    <span>￥</span>
                    <span>{{i.amount}}</span>
                  </p>
                  <p>{{i.description_map.sum_condition}}</p>
                </div>
                <div class="list_item_right">
                  <h4>{{i.name}}</h4>
                  <p>{{i.end_date}}到期</p>
                  <p>{{i.description_map.phone}}</p>
                </div>
                <div class="time_left">{{i.description_map.validity_delta}}</div>
              </div>
            </div>
          </div>
          <div class="list_item_footer">
            <p>限品类：快餐便当、特色菜系、小吃夜宵、甜品饮品、异国料理</p>
          </div>
          <div class="history_hongbao">
            <a>
              <span>查看历史红包&gt;</span>
            </a>
          </div>
          <div class="hongbao_footer">
            <span><router-link to="/discounts/exchange">兑换红包</router-link></span>
            <span><router-link to="/discounts/commend">推荐有奖</router-link></span>
          </div>
        </div>
      </div>
      <!-- 商家代金券 -->
      <div v-if="index==1">
        <div class="voucher_container">
          <div class="hongbao_description voucher_header">
            <img
              src="data:image/png;base64,"
            />
            <a class="hongbao_detail">
              <router-link to="/discounts/coupon">商家代金券说明</router-link>
              </a>
          </div>
          <div class="unable_use">
            <p>
              <img
                src="data:image/png;base64,"
              />
            </p>
            <p>无法使用代金券</p>
            <p>非客户端或客户端版本过低</p>
            <a class="download_app"><router-link to="/discounts/download1" class="download_a">下载或升级客户端</router-link></a>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import headtop from "./zu/headtop.vue";
export default {
  name: "discounts",
  components: {
    headtop
  },
  data() {
    return {
      index: 0,
      arr_tab: ["红包", "商家代金券"],
      arr: ""
    };
  },
  created() {
    this.$http
      .get(
        "http://elm.cangdu.org/promotion/v2/users/44084/hongbaos?limit=20&offset=0"
      )
      .then(data => {
        console.log(data.data);
        this.arr = data.data;
      });
  }
};
</script>

<style scoped>
.arrow-lefts {
  color: #fff;
  line-height: 80px;
  margin-left: 20px;
}
.arrow-downup .van-icon .van-icon-arrow-left {
  padding-left: 15px;
}
.title_head1 {
  font-size: 26px;
  font-weight: 700;
}
.category_title {
  margin-top: 80px;
  overflow: hidden;
  width: 100%;
  height: 80px;
  background: #fff;
}
.tab {
  width: 50%;
  float: left;
  font-size: 26px;
}
.tab .ou {
  border-bottom: 6px #3190e8 solid;
  color: #3190e8;
}
.tab span {
  height: 60px;
  line-height: 60px;
  display: inline-block;
  color: #333;
}
.hongbao_container {
  width: 90%;
  height: auto;
  margin: 0 auto;
}
.hongbao_bigc {
  width: 100%;
}
.hongbao_title {
  width: 100%;
  overflow: hidden;
  height: 80px;
  line-height: 80px;
  font-size: 20px;
}
.total_number {
  float: left;
  color: #666;
}
.total_number span {
  color: #ff5340;
}
.hongbao_description {
  float: right;
}
.hongbao_description img {
  width: 24px;
  height: 24px;
  vertical-align: middle;
  margin-right: 4px;
}
.hongbao_detail {
  color: #3190e8;
}
.hongbao_list_li {
  border-top: 10px red solid;
  box-sizing: border-box;
  margin-bottom: 20px;
  border-radius: 6px;
  background: #fff;
}
.list_item {
  justify-content: space-between;
  padding: 30px 5% 5%;
  overflow: hidden;
}
.list_item_left {
  float: left;
  width: 25%;
  line-height: 40px;
  border-right: 1px dotted #ccc;
  overflow: hidden;
}
.list_item_left span:first-of-type {
  font-size: 30px;
  color: #ff5340;
  font-weight: 400;
}
.list_item_left span:nth-of-type(2) {
  font-size: 50px;
  color: #ff5340;
  font-weight: 400;
  overflow: hidden;
}
.list_item_left span:nth-of-type(3) {
  font-size: 30px;
  color: #ff5340;
  font-weight: 400;
}
.list_item_left span:nth-of-type(4) {
  font-size: 30px;
  color: #ff5340;
  font-weight: 400;
}
.list_item_left p {
  font-size: 20px;
  color: #999;
}
.list_item_right {
  float: left;
  width: 60%;
  padding-left: 10%;
  box-sizing: border-box;
}
.list_item_right h4 {
  font-size: 30px;
  color: #666;
}
.list_item_right p {
  font-size: 16px;
  color: #999;
}
.time_left {
  font-size: 28px;
  color: #ff5340;
  float: right;
}
.list_item_footer {
  font-size: 16px;
  color: #999;
  margin: 0 auto;
  text-align: center;
  line-height: 50px;
}
.list_item_footer p {
  color: #999;
}
.history_hongbao {
  text-align: center;
  margin-top: 20px;
}
.hongbao_footer {
    position: fixed;
    bottom: 0;
    width: 100%;
    left:0;
    background-color: #fff;
    overflow: hidden;
}
.hongbao_footer span{
  width:50%;
  height:80px;
  line-height:80px;
  float:left;
  text-align:center;
  color:#000;
}
.hongbao_footer span:first-of-type{
  border-right:1px #ccc solid;
  box-sizing: border-box
}
.voucher_container .voucher_header {
  font-size: 20px;
  line-height: 80px;
}
.unable_use {
  padding-top: 400px;
}
.unable_use p {
  text-align: center;
}
.unable_use p img {
  width: 240px;
  height: 140px;
}
.unable_use p:nth-of-type(2) {
  font-size: 28px;
  color: #666;
  margin-top: 10px;
}
.unable_use p:nth-of-type(3) {
  font-size: 20px;
  color: #999;
  margin-top: 10px;
  margin-bottom: 20px;
}
.download_app {
  background-color: #56d176;
  font-size: 24px;
  color: #fff;
  padding: 15px;
  display: block;
  border-radius: 6px;
  text-align: center;
  width: 260px;
  margin: 0 auto;
}
.download_a{
  color: #fff;
}
</style>